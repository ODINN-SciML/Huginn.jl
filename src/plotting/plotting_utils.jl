...
function plot_analysis_flow_parameters(
        simulation::SIM, A_values, n_values) where {SIM <: Simulation}
    ...
    results = [generate_result(simulation, A_values[j], n_values[i])
    ...
    max_abs_value = max(abs(minimum(reduce(vcat, [vec(matrix) for matrix in h_diff]))),
        abs(maximum(reduce(vcat, [vec(matrix) for matrix in h_diff]))))
    ...
    for i in 1:rows
        for j in 1:cols
            ...
            hm_diff = Makie.heatmap!(
                ax_diff, Sleipnir.reverseForHeatmap(h_diff[i, j], x, y),
                colormap = :redsblues, colorrange = (-max_abs_value, max_abs_value))
            ...
            Makie.poly!(ax_diff,
                Rect(nx - round(0.15*nx), round(0.075*ny), scale_width, scale_width/10),
                color = :black)
            Makie.text!(ax_diff, "$scale_number km",
                position = (
                    nx - round(0.15*nx)+scale_width/16, round(0.075*ny)+scale_width/10),
                fontsize = textsize)
    ...
    fig[0, :] = Label(fig,
        "Ice Thickness difference ΔH for varying A and n from $start_year to $end_year")

    fig[rows + 1, :] = Label(fig,
        "$rgi_id - latitude = $(round(lat;digits=6)) ° - longitude = $(round(lon;digits=6)) °")
    return fig
end
