...
"""
function avg!(O, I)
    @views @. O = (I[1:(end - 1), 1:(end - 1)] + I[2:end, 1:(end - 1)] +
                   I[1:(end - 1), 2:end] + I[2:end, 2:end]) * 0.25
end
...
"""
@views avg(A) = 0.25 .* (A[1:(end - 1), 1:(end - 1)] .+ A[2:end, 1:(end - 1)] .+
...
function d2dxy(f::Matrix{T}, i::Int, j::Int, Δx::Float64, Δy::Float64) where {T <: Real}
    return (f[i + 1, j + 1] - f[i + 1, j - 1] - f[i - 1, j + 1] + f[i - 1, j - 1]) /
           (4 * Δx * Δy)
end
