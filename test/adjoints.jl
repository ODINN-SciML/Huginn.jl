...
function test_adjoint_diff()
    ...
    for i in range(1, 5)
        ...
        Aadjv = Huginn.diff_y_adjoint(v, Δ)
        a=sum(Au.*v)/fac
        b=sum(u.*Aadjv)/fac
        @test a≈b rtol=(Sleipnir.doublePrec ? 1e-12 : 1e-5)
    end
...
function test_adjoint_clamp_borders()
    ...
    for i in range(1, 5)
        ...
        c = Huginn.clamp_borders_dx(dS, H, η₀, Δ)
        ∂dS = zero(dS)
        ∂H = zero(H)
    ...
    for i in range(1, 5)
        ...
        c = Huginn.clamp_borders_dy(dS, H, η₀, Δ)
        ∂dS = zero(dS)
        ∂H = zero(H)
        Huginn.clamp_borders_dy_adjoint!(∂dS, ∂H, v, η₀, Δ, H, dS)
        a=sum(c.*v)/fac
        b=sum(H.*∂H)/fac + sum(dS.*∂dS)/fac
        @test a≈b rtol=(Sleipnir.doublePrec ? 1e-12 : 1e-5)
    end
...
function test_adjoint_SIAD2D()
    ...
    for k in range(17,21)
        ...
        ∂A_num = compute_numerical_gradient(Avec, (H, simulation, t, vecBackwardSIA2D), f_A, ϵ)
        ratio_k, angle_k, relerr_k = stats_err_backward([∂A], ∂A_num)
        push!(ratio, ratio_k)
        push!(angle, angle_k)
        push!(relerr, relerr_k)
    end
